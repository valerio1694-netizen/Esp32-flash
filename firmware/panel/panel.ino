...
// Touch (XPT2046)
static constexpr int TOUCH_CS  = 27;  // Touch Chip Select
// T_IRQ wird nicht genutzt (Polling)

// ===================== LovyanGFX: nur Display =====================
...
void drawHeader() {
  ...
  lcd.setCursor(10, 10);
  lcd.println("XPT2046 RAW TEST (Fixed)");
...
void loop() {
  ...
  if (xpt2046_readXY(rx, ry)) {
    ...
    // === Fix: Achsen getauscht + X gespiegelt ===
    int sx = mapClamp((int)ry, RAW_MIN, RAW_MAX, 0, 319);  // vertauschte Achsen
    int sy = mapClamp((int)rx, RAW_MIN, RAW_MAX, 0, 479);
    sx = 319 - sx;  // Spiegelung auf X-Achse
